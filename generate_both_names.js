...
const alternateNamesDict = require('./alternateNames/alternateNames_zh_hant_dict.json')
...
// const THRES_CITY = Math.sqrt(500000)
// const THRES_L = Math.sqrt(250000)
// const THRES_M = Math.sqrt(100000)
...
const main = async () => {
    ...
    const outFile = 'EU_edit_both_names.csv'
    const result = []
    const failed = []
    ...
    rl.on('line', (line) => {
        ...
        const outSize = (outPopulation > THRES_L) ? 'L' : (outPopulation > THRES_M) ? 'M' : 'S'
        const outCity = (outPopulation > THRES_CITY) ? '1' : '0'
        count[outSize] += 1
